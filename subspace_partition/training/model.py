...
class NewUnevenRTrainer(nn.Module):
    def __init__(
        self, h_dim, partition: list[int], cfg, buffer: BufferReuse, previous_R=None
    ):
        super().__init__()
    ...
    def _get_subspace_h(self, h, dtype=torch.float32):
        ...
        for i in range(len(h)):
            h[i] = (
                h[i]
                .view(bz, self._partition[i][1], self._partition[i][0])
                .transpose(0, 1)
            )  # num_unit, bz, unit_size
        ...
        if self.metric == "cosine":
            with torch.no_grad():
                ...
                query_h_normed = [
                    (
                        h
                        / torch.linalg.vector_norm(h, dim=-1, keepdim=True).clamp(
                            min=1e-8
                        )
                    ).to(search_dtype)
                    for h in query_h
                ]
                most_sim_value = -torch.ones(
                    num_unit, bz, dtype=search_dtype, device=device
                )
                most_sim_vec = torch.zeros(
                    num_unit,
                    bz,
                    self.h_dim,
                    dtype=self.buffer.buffer_dtype,
                    device=device,
                )
            ...
            most_sim_vec = self.R(most_sim_vec.flatten(end_dim=1), torch.float32).view(
                num_unit, bz, self.h_dim
            )
            ...
            for i, chunk in enumerate(
                most_sim_vec.split([num for _, num in self._partition], dim=0)
            ):
                d, n = self._partition[i]
                chunk = (
                    chunk[:, :, temp[i] : temp[i + 1]].view(n, bz, n, d).transpose(1, 2)
                )
            ...
            with torch.no_grad():
                smallest_dist = torch.full(
                    (num_unit, bz),
                    torch.finfo(search_dtype).max,
                    dtype=search_dtype,
                    device=device,
                )
                most_sim_vec = torch.zeros(
    ...
    def evaluate_step(self, num_steps=None, batch_size=None):
        ...
        if self.metric == "cosine":
            query_h = [
                h / torch.linalg.vector_norm(h, dim=-1, keepdim=True).clamp(min=1e-8)
                for h in query_h
            ]
            ...
            for i in range(num_steps):
                ...
                key_h = [
                    h
                    / torch.linalg.vector_norm(h, dim=-1, keepdim=True).clamp(min=1e-3)
                    for h in key_h
                ]
    ...
    def compute_MI_step(
        ...
        constant = torch.special.digamma(
            torch.ones((), device=device)
        ) + torch.special.digamma(torch.tensor(N, device=device))
        I = (
            constant - torch.special.digamma(n_x + 1) - torch.special.digamma(n_y + 1)
        ).mean(dim=-1)
    ...
    def from_trained(cls, path, cfg, buffer, suffix=""):
        ...
        config["previous_R"] = torch.load(path / f"R{suffix}.pt", map_location="cpu")[
...
class NewSplitRTrainer(nn.Module):
    def __init__(
        self,
        h_dim,
        prev_partition: list[int],
        previous_R: torch.FloatTensor,
        new_partition: list[list[tuple[int]]],
        cfg,
        buffer: BufferReuse,
    ):
        super().__init__()
        ...
        assert all(
            len(prev_partition) == len(new_partition[i])
                ...
                nn.ModuleList(
                    [
                        nn.utils.parametrizations.orthogonal(
                            rotateMatrix(get_random_init_matrix(d))
                        )
    ...
    def _get_subspace_h(self, h, trial_idx, dtype=torch.float32):
        ...
        subspace_h = []
        for h_chunk, R, split_dim in zip(
            h, self.Rs[trial_idx], self.new_partition[trial_idx]
        ):
        ...
        for trial_idx in range(self.num_split_trial):
            ...
            if self.metric == "cosine":
                with torch.no_grad():
                    ...
                    most_sim_value = -torch.ones(
                        num_unit, bz, dtype=torch.float, device=device
                    )
                    most_sim_vec = torch.zeros(
                        num_unit,
                        bz,
                ...
                most_sim_h = self._get_subspace_h(
                    most_sim_vec.flatten(end_dim=1), trial_idx
                )
                sim = 0
                for i in range(len(most_sim_h)):
                    subspace_sim = F.cosine_similarity(
                        query_h[i],
                        most_sim_h[i].view(num_unit, bz, most_sim_h[i].size(-1))[i],
                        dim=-1,
                    )
                ...
                dist = 0
                for i in range(len(most_sim_h)):
                    subspace_dist = torch.linalg.vector_norm(
                        query_h[i]
                        - most_sim_h[i].view(num_unit, bz, most_sim_h[i].size(-1))[i],
                        dim=-1,
                    )
    ...
    def evaluate_step(self, num_steps=None):
        ...
        for trial_idx in range(self.num_split_trial):
            ...
            if self.metric == "cosine":
                query_h = [
                    h
                    / torch.linalg.vector_norm(h, dim=-1, keepdim=True).clamp(min=1e-8)
                    for h in query_h
                ]
                most_sim_value = -torch.ones(
                    num_unit, bz, dtype=torch.float, device=device
    ...
    def compute_subspace_var(self, num=2000):
        ...
        for trial_idx in range(self.num_split_trial):
            ...
            var = torch.stack(
                [subspace_h.var(dim=0).sum() for subspace_h in all_subspace_h]
            )
            all_var.append(var)
    ...
    def compute_MI_step(self, metric="euclidean", num_steps=None, subspace_var=None):
        ...
        for trial_idx in range(self.num_split_trial):
            ...
            smallest_dist_per_space = torch.full(
                (num_unit, bz),
                torch.finfo(torch.float).max,
                dtype=torch.float,
                device=device,
            ...
            if num_steps is None:
                max_num_steps = -(
                    self.search_num[trial_idx].max().item() // (-self.buffer.block_len)
                )
                num_steps = min(max_num_steps, self.num_steps)
            ...
            for i in range(num_steps):
                ...
                for j, k in pairs:
                    n_x_y[(j, k)]["n_x"] += (
                        dist[j] < smallest_dist[(j, k)].unsqueeze(-1)
                    ).sum(dim=-1)
                    n_x_y[(j, k)]["n_y"] += (
                        dist[k] < smallest_dist[(j, k)].unsqueeze(-1)
                    ).sum(dim=-1)
            ...
            for j, k in pairs:
                I.append(
                    (
                        constant
                        - torch.special.digamma(n_x_y[(j, k)]["n_x"] + 1)
                        - torch.special.digamma(n_x_y[(j, k)]["n_y"] + 1)
                    ).mean()
                )
