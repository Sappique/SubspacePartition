...
def make_histogram(_index: faiss.IndexFlat, sel_subspace, act_idx):
    ...
    counts, bin_edges = np.histogram(
        D, bins=100, range=(-1.0, 1.0) if cosine else (0.0, 3.0)
    )
    fig = go.Figure(go.Bar(x=bin_edges[:-1], y=counts, width=np.diff(bin_edges)))
    fig.update_layout(
        title=f"Histogram of {'sim' if cosine else 'dist'} given query act {act_idx} (mean {np.mean(D).item():.2f})",
        width=600,
        height=300,
    )
    return fig
...
def span_maker(token: str, mark: bool = False):
    if mark:
        return (
            '<span style="background-color:rgb(238,75,43); color: white">'
            + token
            + "</span>"
        )
    else:
...
cached_input, seq_edges = get_cached_input(
    Path(args.cached_acts_path) / f"shared_acts-{model_name}"
)
...
with st.sidebar:
    ...
    if (
        "sel_subspace" not in st.session_state
        or st.session_state.sel_subspace not in subspaces
    ):
        st.session_state.sel_subspace = subspaces[0]
    sel_subspace = st.selectbox("choose subspace", subspaces, key="sel_subspace")
    cols = st.columns(2)
    cols[0].button(
        "prev", on_click=change_key_value, args=("sel_subspace", subspaces, -1)
    )
    cols[1].button(
        "next", on_click=change_key_value, args=("sel_subspace", subspaces, 1)
    )

    index = get_built_index(
        str(index_path / f"{model_name}-{sel_act_site}" / f"{sel_subspace}.index")
    )
    cosine = isinstance(index, faiss.IndexFlatIP)

    if (
        "sel_example_idx" not in st.session_state
    ) or st.session_state.sel_example_idx >= index.ntotal:
        st.session_state.sel_example_idx = random.randint(0, index.ntotal - 1)
    act_idx = st.number_input(
        "query act idx",
        min_value=0,
        max_value=index.ntotal - 1,
        value=st.session_state.sel_example_idx,
    )
    st.button(
        "random example", type="primary", on_click=click_random, args=(index.ntotal,)
    )  # can choose the first half
    ...
    if cosine:
        threshold = st.number_input(
            "set threshold", min_value=-1.0, max_value=0.999, value=0.85
        )
    else:
        threshold = st.number_input(
            "set threshold", min_value=0.001, max_value=1.0, value=0.25
        )
    num_show = st.slider("num sample shown", min_value=1, max_value=100, value=20)
    prev_ctx = st.select_slider(
        "# prev context per sample", ["10", "25", "100", "inf"], value="10"
    )
    futr_ctx = st.select_slider(
        "# future context per sample", ["0", "5", "25", "inf"], value="5"
    )
...
D, I = search_for_idx(
    index, model_name + sel_act_site + sel_subspace, act_idx, threshold
)
...
if show_explanation:
    st.write("#####")
    st.caption(
        """_Token in red_ is the token position where the activation is taken from. Activations are projected to selected subspace (e.g., 2-96 means subspace 2 of dimension 96),
